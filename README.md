...
- Hover a player slot and use modifier+`L` to lock it or modifier+`F` to toggle that item ID as a favourite. Use Command or Option on macOS and Ctrl or Alt elsewhere. Locked and favourite slots are also protected from shift-click quick deposit.
- Use modifier+`H` over a hotbar slot to remember its current item, or modifier+`N` over any player slot to configure automatic network restocking. Slot markers show the active rules and name a nearby network that can currently supply a configured item.
