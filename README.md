...
- The compact terminal browses six rows of the combined loaded inventory, crafts in a 3×3 grid, and filters inline with the label expression language. Left-side controls sort by item, quantity, or mod in either direction and support exact-stack withdrawal. The terminal also tracks loaded nodes and used capacity. Recipe-transfer helpers use live physical network slots, so whole and partial ingredient transfers remove their backing items even when ingredients are off-page.
